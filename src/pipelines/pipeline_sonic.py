...
from src.distributed.util import gather_forward, get_rank, get_world_size
    ...
    with self.progress_bar(total=num_inference_steps) as progress_bar:
        for i, t in enumerate(timesteps):
            ...
            for batch, index_start in enumerate(range(0, num_frames, frames_per_batch - overlap)):
                ...
                # Cutting tensor
                # latent
                latent_model_input_rank = torch.chunk(latent_model_input, get_world_size(), dim=1)[get_rank()]
                del latent_model_input

                # batch_image_embedding
                batch_image_embeddings_rank = torch.chunk(batch_image_embeddings, get_world_size(), dim=1)[get_rank()]
                del batch_image_embeddings

                # batch_audio_prompts
                batch_audio_prompts_rank = torch.chunk(batch_audio_prompts, get_world_size(), dim=1)[get_rank()]
                del batch_audio_prompts


                # predict the noise residual
                noise_pred = self.unet(
                    latent_model_input_rank,
                    t,
                    encoder_hidden_states=(batch_image_embeddings_rank.flatten(0,1), [batch_audio_prompts_rank.flatten(0,1)]),
                ...
                # gather
                noise_pred = gather_forward(noise_pred , dim=1)
